    ...
    <div className="mb-6">
      ...
      <div className="grid grid-cols-7 gap-1">
        {days.map((d, idx) => {
          ...
          return (
            <button key={idx} type="button" disabled={!isAdmin}
              className={`${base} ${cls} ${!isAdmin?'opacity-90':''}`}
              onClick={()=> free ? onTapFree(room.id, d) : onTapBooked(room.id, d)}>
              {booked && (()=>{
...
export default function Availability({ isAdmin }){
  ...
  function openCreate(roomId, day){ if(!isAdmin) return; setModal({ open:true, mode:"create", roomId, bookingId:null, start: formatISODate(day), end: formatISODate(addDays(day,1)), guest:"", note:"" }); }
  function openEdit(roomId, day){
    ...
    if(!b) return; if(!isAdmin) return;
    setModal({ open:true, mode:"edit", roomId, bookingId:b.id, start: formatISODate(new Date(b.start)), end: formatISODate(new Date(b.end)), guest: b.guest||"", note: b.note||"" });
  }
  ...
  async function saveModal(){
    ...
    if(modal.mode==="create"){
      ...
      setBookings(prev => { const arr=[...prev, newBk]; try{ localStorage.setItem("ghc_cloud_cache_v1", JSON.stringify(arr)); }catch{} return arr; });
      try {
        ...
        setBookings(prev => { const arr=prev.map(b => b.id===tempId ? { ...newBk, id: ref.id } : b); try{ localStorage.setItem("ghc_cloud_cache_v1", JSON.stringify(arr)); }catch{} return arr; });
        setSyncError(null);
      ...
      setBookings(prev => { const arr=prev.map(b => b.id===edited.id ? edited : b); try{ localStorage.setItem("ghc_cloud_cache_v1", JSON.stringify(arr)); }catch{} return arr; });
      try {
  ...
  async function cancelBooking(){
    ...
    setModal(m=> ({...m, open:false}));
    setBookings(prev => { const arr=prev.filter(b => b.id !== id); try{ localStorage.setItem("ghc_cloud_cache_v1", JSON.stringify(arr)); }catch{} return arr; });
